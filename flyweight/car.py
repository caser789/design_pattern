...
class CarModel(object):
    ...
    def __new__(cls, model_name, *args, **kw):
        model = cls._models.get(model_name)
        if not model:
            model = super(CarModel, cls).__new__(cls)
    ...
    def check_serial(self, serial_number):
        print "Sorry, we are unable to check the serial number {0} on the {1} at the time".format(
